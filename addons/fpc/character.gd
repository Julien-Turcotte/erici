...
## Controls the intensity of the headbob effect.
@export_range(0.0, 2.0, 0.01) var headbob_amount : float = 1.0
...
# Store the original headbob animation key values so slider changes stay consistent.
var _headbob_animation_default_values : Dictionary = {}
...
func check_controls(): # If you add a control, you might want to add a check for it here.
	# Create fallback input actions if they are missing so the controller works out of the box.
	_ensure_input_action(controls.LEFT, [Key.A, Key.LEFT])
	_ensure_input_action(controls.RIGHT, [Key.D, Key.RIGHT])
	_ensure_input_action(controls.FORWARD, [Key.W, Key.UP])
	_ensure_input_action(controls.BACKWARD, [Key.S, Key.DOWN])
	_ensure_input_action(controls.JUMP, [Key.SPACE])
	_ensure_input_action(controls.PAUSE, [Key.ESCAPE])
	_ensure_input_action(controls.CROUCH, [Key.C])
	_ensure_input_action(controls.SPRINT, [Key.SHIFT_LEFT, Key.SHIFT_RIGHT])
...
func initialize_animations():
	...
	apply_headbob_amount()

func set_headbob_amount(value):
	headbob_amount = value
	apply_headbob_amount()

func apply_headbob_amount():
	if not HEADBOB_ANIMATION:
		return

	var animation_names = ["walk", "sprint"]
	for animation_name in animation_names:
		if not HEADBOB_ANIMATION.has_animation(animation_name):
			continue

		var animation = HEADBOB_ANIMATION.get_animation(animation_name)
		if animation == null:
			continue

		if not _headbob_animation_default_values.has(animation_name):
			var track_defaults : Array = []
			for track_index in range(animation.get_track_count()):
				var track_path = animation.track_get_path(track_index)
				if track_path == NodePath("Camera:position:x") or track_path == NodePath("Camera:position:y"):
					var key_defaults : Array = []
					for key_index in range(animation.track_get_key_count(track_index)):
						key_defaults.append(animation.track_get_key_value(track_index, key_index))
					track_defaults.append({"index": track_index, "values": key_defaults})
			_headbob_animation_default_values[animation_name] = track_defaults

		for track_data in _headbob_animation_default_values[animation_name]:
			var track_index = track_data["index"]
			for key_index in range(track_data["values"].size()):
				var original_value = track_data["values"][key_index]
				animation.track_set_key_value(track_index, key_index, original_value * headbob_amount)
